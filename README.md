...
The GitHub `Verify` workflow runs the tests on Java 21/Linux and validates the container and Terraform. Its [hosted verification](https://github.com/RyGuy108/inventory-cloud-service/actions/runs/34898299416) passed 63 Java tests, 121 operational tests, five mocked Terraform tests, and the Linux AMD64 image/compatibility/security checks. The repository root is this project directory; see [GitHub delivery](docs/github-delivery.md) for the publication setup and [verification evidence](reports/github/verification.json) for the exact source commit and image.

All seven drills also passed in the [hosted operational workflow](https://github.com/RyGuy108/inventory-cloud-service/actions/runs/34898295610) against its own tested and scanned image. Its [combined evidence](reports/github/operations.json) records authentication, restore, password rotation, load, two-instance restart, image rollback, and monitoring results. This is verification on disposable Linux containers; the AWS application environment is still unprovisioned.
